...
TheEngine::DeviceContext::DeviceContext(const GraphicsResourceDesc& gDesc): GraphicsResource(gDesc)
{

	THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateDeferredContext(0,&m_context),
...
void TheEngine::DeviceContext::createBlendStates()
{
	...
	THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateBlendState(&blendDesc, m_alphaBlendState.GetAddressOf()),
		"Failed to create alpha blend state");

	// No blend (default opaque)
	blendDesc.RenderTarget[0].BlendEnable = FALSE;
	THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateBlendState(&blendDesc, m_noBlendState.GetAddressOf()),
		"Failed to create no-blend state");
}

void TheEngine::DeviceContext::createDepthStates()
{
	...
	THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateDepthStencilState(&depthDesc, m_defaultDepthState.GetAddressOf()),
		"Failed to create default depth state");

	// Transparent depth (disable writes)
	depthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateDepthStencilState(&depthDesc, m_transparentDepthState.GetAddressOf()),
		"Failed to create transparent depth state");
}

// Enable alpha blending
void TheEngine::DeviceContext::enableAlphaBlending()
{
...
void TheEngine::DeviceContext::disableAlphaBlending()
{
	float blendFactor[4] = { 0,0,0,0 };
...
void TheEngine::DeviceContext::enableTransparentDepth()
{
	m_context->OMSetDepthStencilState(m_transparentDepthState.Get(), 0);
}

// Reset to default depth
void TheEngine::DeviceContext::enableDefaultDepth()
{
	m_context->OMSetDepthStencilState(m_defaultDepthState.Get(), 0);
}
// 2. Implementation in DeviceContext.cpp
void TheEngine::DeviceContext::setScreenSpaceMatrices(float screenWidth, float screenHeight) {
...
void TheEngine::DeviceContext::restoreWorldSpaceMatrices(const Mat4& viewMatrix, const Mat4& projectionMatrix) {
	setViewMatrix(viewMatrix);
	setProjectionMatrix(projectionMatrix);
}
void TheEngine::DeviceContext::createConstantBuffers()
{
...
void TheEngine::DeviceContext::setTint(const Vec4& tint)
{
...
void TheEngine::DeviceContext::setWorldMatrix(const Mat4& worldMatrix)
{
	m_currentTransforms.worldMatrix = worldMatrix;
	updateTransformBuffer();
}

void TheEngine::DeviceContext::setViewMatrix(const Mat4& viewMatrix)
{
	m_currentTransforms.viewMatrix = viewMatrix;
	updateTransformBuffer();
}

void TheEngine::DeviceContext::setProjectionMatrix(const Mat4& projectionMatrix)
{
	m_currentTransforms.projectionMatrix = projectionMatrix;
	updateTransformBuffer();
}

void TheEngine::DeviceContext::setSampler(ui32 slot, NativeGraphicsHandle sampler)
{
	auto* nativeSampler = static_cast<ID3D11SamplerState*>(sampler);
	m_context->PSSetSamplers(slot, 1, &nativeSampler);
}

void TheEngine::DeviceContext::setDirectionalLight(const Vec3& direction, const Vec3& color, float intensity, float ambient)
{
...
void TheEngine::DeviceContext::setLights(const std::vector<Vec3>& dirs, const std::vector<Vec3>& colors, const std::vector<float>& intensities)
{
	struct Header { ui32 count; float pad[3]; };
	struct PackedLight { Vec3 dir; float intensity; Vec3 color; float pad0; };
    struct Buffer { Header h; PackedLight l[10]; };
...
void TheEngine::DeviceContext::setMaterial(const Vec3& specColor, float shininess, float ambient)
{
...
void TheEngine::DeviceContext::setCameraPosition(const Vec3& pos)
{
...
void TheEngine::DeviceContext::setPBR(bool enabled, const Vec3& albedo, float metallic, float roughness)
{
...
void TheEngine::DeviceContext::setSpotlight(bool enabled, const Vec3& position, const Vec3& direction, float range,
...
void TheEngine::DeviceContext::clearAndSetBackBuffer(const IRenderSwapChain& swapChain, const Vec4& color)
{
...
void TheEngine::DeviceContext::setGraphicsPipelineState(const IRenderPipelineState& pipeline)
{
...
void TheEngine::DeviceContext::setVertexBuffer(const IRenderVertexBuffer& buffer)
{
...
void TheEngine::DeviceContext::setViewportSize(const Rect& size)
{
	D3D11_VIEWPORT vp{};
...
void TheEngine::DeviceContext::disableDepthTest()
{
	// Disable depth
	D3D11_DEPTH_STENCIL_DESC depthDesc = {};
...
void TheEngine::DeviceContext::enableDepthTest()
{
	enableDefaultDepth(); 
}
void TheEngine::DeviceContext::drawTriangleList(ui32 vertexCount, ui32 startVertexLocation)
{
	m_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	m_context->Draw(vertexCount, startVertexLocation);
}

void TheEngine::DeviceContext::setIndexBuffer(IRenderIndexBuffer& ib, IndexFormat fmt, ui32 offset)
{
...
void TheEngine::DeviceContext::drawIndexedTriangleList(ui32 indexCount, ui32 startIndex)
{
	m_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
void TheEngine::DeviceContext::drawIndexedLineList(ui32 indexCount, ui32 startIndex)
{
...
void TheEngine::DeviceContext::setTexture(ui32 slot, NativeGraphicsHandle srv)
{
	auto* nativeSrv = static_cast<ID3D11ShaderResourceView*>(srv);
	m_context->PSSetShaderResources(slot, 1, &nativeSrv);
}

void TheEngine::DeviceContext::setPSConstants0(const void* data, ui32 byteSize)
{
    ...
    if (!s_psB0 || aligned > s_capacity) {
        ...
        s_psB0.Reset();
        THEENGINE_GRAPHICS_LOG_THROW_ON_FAIL(m_device.CreateBuffer(&bd, nullptr, s_psB0.GetAddressOf()),
...
void TheEngine::DeviceContext::updateTransformBuffer()
{
...
void TheEngine::DeviceContext::setShadowMap(NativeGraphicsHandle shadowMap, NativeGraphicsHandle shadowSampler)
{
...
void TheEngine::DeviceContext::setShadowMaps(NativeGraphicsHandle const* shadowMaps, ui32 count, NativeGraphicsHandle shadowSampler)
{
...
void TheEngine::DeviceContext::setShadowMatrix(const Mat4& lightViewProj)
{
	D3D11_MAPPED_SUBRESOURCE mapped{};
...
void TheEngine::DeviceContext::setShadowMatrices(const std::vector<Mat4>& lightViewProjMatrices)
{
    D3D11_MAPPED_SUBRESOURCE mapped{};
    HRESULT hr = m_context->Map(m_shadowBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
    if (SUCCEEDED(hr)) {
...
void TheEngine::DeviceContext::clearShaderResourceBindings()
{
