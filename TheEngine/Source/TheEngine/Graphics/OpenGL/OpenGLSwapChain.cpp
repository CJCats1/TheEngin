	...
	: m_logger(logger), m_size(desc.winSize)
{
	...
	if (!glfwInit())
	{
		THEENGINE_LOG_THROW(m_logger, std::runtime_error, Logger::LogLevel::Error, "GLFW init failed.");
	}
	...
	if (!window)
	{
		THEENGINE_LOG_THROW(m_logger, std::runtime_error, Logger::LogLevel::Error, "GLFW window creation failed.");
	}

	glfwMakeContextCurrent(window);
	if (!gladLoadGLLoader(reinterpret_cast<GLADloadproc>(glfwGetProcAddress)))
	{
		THEENGINE_LOG_THROW(m_logger, std::runtime_error, Logger::LogLevel::Error, "OpenGL loader init failed.");
	}
