...
std::shared_ptr<Mesh> Mesh::CreateQuadColored(IRenderDevice& device, float w, float h)
{
    ...
    auto whiteTexture = TheEngine::Texture2D::CreateDebugTexture(device);
...
std::shared_ptr<Mesh> Mesh::CreateFromOBJ(IRenderDevice& device, const std::string& path)
{
    ...
    if (!chosenDiffuseTexPath.empty()) {
        ...
        auto tex = TheEngine::Texture2D::LoadTexture2D(device, wpath.c_str());
        if (tex) m->setTexture(tex);
        else m->setTexture(TheEngine::Texture2D::CreateDebugTexture(device));
    } else {
        m->setTexture(TheEngine::Texture2D::CreateDebugTexture(device));
    }
    return m;
}
