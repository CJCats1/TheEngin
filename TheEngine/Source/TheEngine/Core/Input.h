#pragma once
#include <unordered_map>
#include <TheEngine/Math/Geometry.h>
#include <iostream>

#if defined(_WIN32)
#include <Windows.h>
#else
#include <android/keycodes.h>
struct ANativeWindow;
#endif

namespace TheEngine {
#if defined(_WIN32)
    using NativeWindowHandle = HWND;
#else
    using NativeWindowHandle = ANativeWindow*;
#endif

    enum class MouseClick
    {
#if defined(_WIN32)
        LeftMouse = WM_LBUTTONDOWN,
        RightMouse = WM_RBUTTONDOWN,
        MiddleMouse = WM_MBUTTONDOWN
#else
        LeftMouse = 1,
        RightMouse = 2,
        MiddleMouse = 3
#endif
    };
    // Key codes enum for better readability (optional, you can still use raw codes)
    enum class Key {
        // Letters
#if defined(_WIN32)
        A = 0x41, B = 0x42, C = 0x43, D = 0x44,
        E = 0x45, F = 0x46, G = 0x47, H = 0x48,
        I = 0x49, J = 0x4A, K = 0x4B, L = 0x4C,
        M = 0x4D, N = 0x4E, O = 0x4F, P = 0x50,
        Q = 0x51, R = 0x52, S = 0x53, T = 0x54,
        U = 0x55, V = 0x56, W = 0x57, X = 0x58,
        Y = 0x59, Z = 0x5A,
#else
        A = AKEYCODE_A, B = AKEYCODE_B, C = AKEYCODE_C, D = AKEYCODE_D,
        E = AKEYCODE_E, F = AKEYCODE_F, G = AKEYCODE_G, H = AKEYCODE_H,
        I = AKEYCODE_I, J = AKEYCODE_J, K = AKEYCODE_K, L = AKEYCODE_L,
        M = AKEYCODE_M, N = AKEYCODE_N, O = AKEYCODE_O, P = AKEYCODE_P,
        Q = AKEYCODE_Q, R = AKEYCODE_R, S = AKEYCODE_S, T = AKEYCODE_T,
        U = AKEYCODE_U, V = AKEYCODE_V, W = AKEYCODE_W, X = AKEYCODE_X,
        Y = AKEYCODE_Y, Z = AKEYCODE_Z,
#endif

        // Numbers (top row)
#if defined(_WIN32)
        Num0 = 0x30, Num1 = 0x31, Num2 = 0x32, Num3 = 0x33,
        Num4 = 0x34, Num5 = 0x35, Num6 = 0x36, Num7 = 0x37,
        Num8 = 0x38, Num9 = 0x39,
#else
        Num0 = AKEYCODE_0, Num1 = AKEYCODE_1, Num2 = AKEYCODE_2, Num3 = AKEYCODE_3,
        Num4 = AKEYCODE_4, Num5 = AKEYCODE_5, Num6 = AKEYCODE_6, Num7 = AKEYCODE_7,
        Num8 = AKEYCODE_8, Num9 = AKEYCODE_9,
#endif

        // Function keys
#if defined(_WIN32)
        F1 = VK_F1, F2 = VK_F2, F3 = VK_F3, F4 = VK_F4,
        F5 = VK_F5, F6 = VK_F6, F7 = VK_F7, F8 = VK_F8,
        F9 = VK_F9, F10 = VK_F10, F11 = VK_F11, F12 = VK_F12,
#else
        F1 = AKEYCODE_F1, F2 = AKEYCODE_F2, F3 = AKEYCODE_F3, F4 = AKEYCODE_F4,
        F5 = AKEYCODE_F5, F6 = AKEYCODE_F6, F7 = AKEYCODE_F7, F8 = AKEYCODE_F8,
        F9 = AKEYCODE_F9, F10 = AKEYCODE_F10, F11 = AKEYCODE_F11, F12 = AKEYCODE_F12,
#endif

        // Modifiers
#if defined(_WIN32)
        Shift = VK_SHIFT,
        Control = VK_CONTROL,
        Alt = VK_MENU,
#else
        Shift = AKEYCODE_SHIFT_LEFT,
        Control = AKEYCODE_CTRL_LEFT,
        Alt = AKEYCODE_ALT_LEFT,
#endif

        // Navigation
#if defined(_WIN32)
        Left = VK_LEFT,
        Right = VK_RIGHT,
        Up = VK_UP,
        Down = VK_DOWN,
        Home = VK_HOME,
        End = VK_END,
        PageUp = VK_PRIOR,
        PageDown = VK_NEXT,
        Insert = VK_INSERT,
        Delete = VK_DELETE,
#else
        Left = AKEYCODE_DPAD_LEFT,
        Right = AKEYCODE_DPAD_RIGHT,
        Up = AKEYCODE_DPAD_UP,
        Down = AKEYCODE_DPAD_DOWN,
        Home = AKEYCODE_HOME,
        End = AKEYCODE_MOVE_END,
        PageUp = AKEYCODE_PAGE_UP,
        PageDown = AKEYCODE_PAGE_DOWN,
        Insert = AKEYCODE_INSERT,
        Delete = AKEYCODE_DEL,
#endif

        // Misc
#if defined(_WIN32)
        Space = VK_SPACE,
        Escape = VK_ESCAPE,
        Enter = VK_RETURN,
        Tab = VK_TAB,
        Backspace = VK_BACK,
        CapsLock = VK_CAPITAL,
#else
        Space = AKEYCODE_SPACE,
        Escape = AKEYCODE_ESCAPE,
        Enter = AKEYCODE_ENTER,
        Tab = AKEYCODE_TAB,
        Backspace = AKEYCODE_DEL,
        CapsLock = AKEYCODE_CAPS_LOCK,
#endif

        // Numpad
#if defined(_WIN32)
        Numpad0 = VK_NUMPAD0, Numpad1 = VK_NUMPAD1, Numpad2 = VK_NUMPAD2,
        Numpad3 = VK_NUMPAD3, Numpad4 = VK_NUMPAD4, Numpad5 = VK_NUMPAD5,
        Numpad6 = VK_NUMPAD6, Numpad7 = VK_NUMPAD7, Numpad8 = VK_NUMPAD8,
        Numpad9 = VK_NUMPAD9,
        NumpadAdd = VK_ADD,
        NumpadSubtract = VK_SUBTRACT,
        NumpadMultiply = VK_MULTIPLY,
        NumpadDivide = VK_DIVIDE,
        NumpadDecimal = VK_DECIMAL,
#else
        Numpad0 = AKEYCODE_NUMPAD_0, Numpad1 = AKEYCODE_NUMPAD_1, Numpad2 = AKEYCODE_NUMPAD_2,
        Numpad3 = AKEYCODE_NUMPAD_3, Numpad4 = AKEYCODE_NUMPAD_4, Numpad5 = AKEYCODE_NUMPAD_5,
        Numpad6 = AKEYCODE_NUMPAD_6, Numpad7 = AKEYCODE_NUMPAD_7, Numpad8 = AKEYCODE_NUMPAD_8,
        Numpad9 = AKEYCODE_NUMPAD_9,
        NumpadAdd = AKEYCODE_NUMPAD_ADD,
        NumpadSubtract = AKEYCODE_NUMPAD_SUBTRACT,
        NumpadMultiply = AKEYCODE_NUMPAD_MULTIPLY,
        NumpadDivide = AKEYCODE_NUMPAD_DIVIDE,
        NumpadDecimal = AKEYCODE_NUMPAD_DOT,
#endif
    };


    class Input {
    public:
        static Input& getInstance();
        void setWindowHandle(NativeWindowHandle hwnd) { m_windowHandle = hwnd; }
        // Called by the game engine when keys are pressed/released
        void setKeyDown(int keyCode);
        void setKeyUp(int keyCode);

        // Query methods for scenes/systems to use
        bool isKeyDown(int keyCode) const;
        bool isKeyDown(Key key) const;

        bool isKeyUp(int keyCode) const;
        bool isKeyUp(Key key) const;

        // Check if key was just pressed this frame (useful for single-press actions)
        bool wasKeyJustPressed(int keyCode) const;
        bool wasKeyJustPressed(Key key) const;

        // Check if key was just released this frame
        bool wasKeyJustReleased(int keyCode) const;
        bool wasKeyJustReleased(Key key) const;

        // Called once per frame to update "just pressed/released" state
        void update();

        // Utility methods
        void reset(); // Clear all input states

        void validateHardwareState()
        {

            if (!m_windowHandle) return;

            // Only check when we regain focus or during display changes
            for (auto& [keyCode, isDown] : m_keyStates) {
                if (isDown) {
#if defined(_WIN32)
                    SHORT keyState = GetAsyncKeyState(keyCode);
                    bool hardwareDown = (keyState & 0x8000) != 0;

                    if (!hardwareDown) {
                        m_keyStates[keyCode] = false;  
                    }
#endif
                }
            }
        }
        // Mouse
        void setMouseDown(MouseClick button);
        void setMouseUp(MouseClick button);

        bool isMouseDown(MouseClick button) const;
        bool isMouseUp(MouseClick button) const;

        bool wasMouseJustPressed(MouseClick button) const;
        bool wasMouseJustReleased(MouseClick button) const;

        Vec2 getMousePosition() const;
        Vec2 getMousePositionScreen() const;
        Vec2 getMousePositionNDC() const;
        Vec2 getMousePositionClient() const;
        void setMousePosition(const Vec2& position);
        inline int MouseClickToVK(TheEngine::MouseClick button)
        {
#if defined(_WIN32)
            switch (button)
            {
            case TheEngine::MouseClick::LeftMouse: return VK_LBUTTON;
            case TheEngine::MouseClick::RightMouse: return VK_RBUTTON;
            case TheEngine::MouseClick::MiddleMouse: return VK_MBUTTON;
            default: return 0;
            }
#else
            (void)button;
            return 0;
#endif
        }

    private:
        NativeWindowHandle m_windowHandle = nullptr;
        Input() = default;
        ~Input() = default;
        Input(const Input&) = delete;
        Input& operator=(const Input&) = delete;

        std::unordered_map<int, bool> m_keyStates;           // Current frame key states
        std::unordered_map<int, bool> m_previousKeyStates;   // Previous frame key states
        std::unordered_map<int, bool> m_justPressed;         // Keys that were just pressed
        std::unordered_map<int, bool> m_justReleased;        // Keys that were just released

        std::unordered_map<MouseClick, bool> m_mouseStates;
        std::unordered_map<MouseClick, bool> m_prevMouseStates;
        std::unordered_map<MouseClick, bool> m_mouseJustPressed;
        std::unordered_map<MouseClick, bool> m_mouseJustReleased;

        Vec2 m_mousePosition{};
    };

    // Convenience macros for common usage patterns
#define INPUT_KEY_DOWN(key) TheEngine::Input::getInstance().isKeyDown(key)
#define INPUT_KEY_UP(key) TheEngine::Input::getInstance().isKeyUp(key)
#define INPUT_KEY_JUST_PRESSED(key) TheEngine::Input::getInstance().wasKeyJustPressed(key)
#define INPUT_KEY_JUST_RELEASED(key) TheEngine::Input::getInstance().wasKeyJustReleased(key)
}