...
void TestScene::renderImGui(GraphicsEngine& engine) {
    ...
    // Temporary business card-style window
    ImGui::Begin("CJ Osborne - Business Card", nullptr, ImGuiWindowFlags_AlwaysAutoResize);

    if (ImGui::CollapsingHeader("Front", ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::TextUnformatted("CJ OSBORNE");
        ImGui::Separator();
        ImGui::TextUnformatted("Software Engineer");
        ImGui::TextUnformatted("Seattle, WA (Willing to Relocate)");
        ImGui::Spacing();
        ImGui::TextUnformatted("C++ / Real-Time Simulation / Graphics");
    }

    if (ImGui::CollapsingHeader("Back")) {
        ImGui::Text("Email: %s", "cj.l.osborne@gmail.com");
        ImGui::Text("Phone: %s", "303-210-9656");
        ImGui::Text("LinkedIn: %s", "linkedin.com/in/cj-osborne-083950301/");
        ImGui::Separator();
        ImGui::TextUnformatted("Highlights");
        ImGui::BulletText("C++ engine, graphics, and tools development");
        ImGui::BulletText("Experience with Unreal and custom engines");
        ImGui::BulletText("B.S. CS (RTIS), DigiPen – May 2026 (expected)");
    }
