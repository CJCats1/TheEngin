	...
	: m_logger(logger), m_size(desc.winSize)
{
	...
	if (!window)
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "ANativeWindow is null.");
	}

	EGLDisplay display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
	if (display == EGL_NO_DISPLAY)
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglGetDisplay failed.");
	}

	if (eglInitialize(display, nullptr, nullptr) == EGL_FALSE)
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglInitialize failed.");
	}
	...
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglChooseConfig failed.");
	}

	EGLSurface surface = eglCreateWindowSurface(display, config, window, nullptr);
	if (surface == EGL_NO_SURFACE)
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglCreateWindowSurface failed.");
	}
	...
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglCreateContext failed.");
	}

	if (eglMakeCurrent(display, surface, surface, context) == EGL_FALSE)
	{
		DX3DLogThrow(m_logger, std::runtime_error, Logger::LogLevel::Error, "eglMakeCurrent failed.");
	}
