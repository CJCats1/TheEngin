    ...
    namespace physics {
        ...
        std::unique_ptr<Hit> AABB::intersectSegment(const Vec2& pos, const Vec2& delta, 
            ...
            if (delta.x == 0.0f) {
                ...
                auto hit = std::make_unique<Hit>(this);
                hit->time = dx3d::clamp(nearTimeY, 0.0f, 1.0f);
                hit->normal.x = 0.0f;
            ...
            if (delta.y == 0.0f) {
                ...
                auto hit = std::make_unique<Hit>(this);
                hit->time = dx3d::clamp(nearTimeX, 0.0f, 1.0f);
            ...
            auto hit = std::make_unique<Hit>(this);
            hit->time = dx3d::clamp(nearTime, 0.0f, 1.0f);
        ...
        std::unique_ptr<Hit> AABB::intersectCircle(const Vec2& center, float radius) const {
            float closestX = dx3d::clamp(center.x, pos.x - half.x, pos.x + half.x);
            float closestY = dx3d::clamp(center.y, pos.y - half.y, pos.y + half.y);
        ...
        Sweep AABB::sweepAABB(const AABB& box, const Vec2& delta) const {
            ...
            if (sweep.hit) {
                sweep.time = dx3d::clamp(sweep.hit->time - EPSILON, 0.0f, 1.0f);
                sweep.pos.x = box.pos.x + delta.x * sweep.time;
                sweep.pos.y = box.pos.y + delta.y * sweep.time;
                
                Vec2 direction = delta;
                direction.normalize();
                sweep.hit->pos.x = dx3d::clamp(
                    sweep.hit->pos.x + direction.x * box.half.x,
                    pos.x - half.x, pos.x + half.x);
                sweep.hit->pos.y = dx3d::clamp(
