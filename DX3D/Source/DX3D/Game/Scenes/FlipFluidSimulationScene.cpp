...
{
    // Visual AABB box using beam.png sprites around domain
    ...
    // Tag boundary sprites for updates
    if (auto* e = m_entityManager->findEntity(boundaryName(0))) if (auto* s = e->getComponent<SpriteComponent>()) s->setRotation(0.0f, 0.0f, m_boxAngle);
    if (auto* e = m_entityManager->findEntity(boundaryName(1))) if (auto* s = e->getComponent<SpriteComponent>()) s->setRotation(0.0f, 0.0f, m_boxAngle);
    if (auto* e = m_entityManager->findEntity(boundaryName(2))) if (auto* s = e->getComponent<SpriteComponent>()) s->setRotation(0.0f, 0.0f, m_boxAngle);
    if (auto* e = m_entityManager->findEntity(boundaryName(3))) if (auto* s = e->getComponent<SpriteComponent>()) s->setRotation(0.0f, 0.0f, m_boxAngle);
...
void FlipFluidSimulationScene::render(GraphicsEngine& engine, SwapChain& swapChain)
{
    ...
    {
        // Draw all sprites (boundaries + particles)
    ...
    // Rotate boundary sprites to match box angle (visual sync) with proper world center rotation
    auto syncBoundarySprite = [&](const std::string& name, const Vec2& localPos)
    {
        if (auto* e = m_entityManager->findEntity(name))
        {
            if (auto* s = e->getComponent<SpriteComponent>())
            {
                ...
                s->setPosition(worldPos.x, worldPos.y, 0.0f);
                s->setRotation(0.0f, 0.0f, m_boxAngle);
            }
        }
    };
    
    // Update each boundary with proper rotation around world center
    syncBoundarySprite(boundaryName(0), Vec2(-m_boxHalf.x + m_boundaryLeftOffset, 0.0f));
    syncBoundarySprite(boundaryName(1), Vec2(m_boxHalf.x + m_boundaryRightOffset, 0.0f));
    syncBoundarySprite(boundaryName(2), Vec2(0.0f, -m_boxHalf.y + m_boundaryBottomOffset));
    syncBoundarySprite(boundaryName(3), Vec2(0.0f, m_boxHalf.y + m_boundaryTopOffset));
}
...
void FlipFluidSimulationScene::renderImGui(GraphicsEngine& engine)
{
    ...
    if (ImGui::Begin("FLIP Fluid (2D)"))
    {
        ...
        {
            m_boundaryLeftOffset = -10.0f;
            m_boundaryRightOffset = 10.0f;
            m_boundaryBottomOffset = -10.0f;
            m_boundaryTopOffset = 10.0f;
        }
...
{
    // Rotated AABB (oriented box) collision
    float c = cosf(m_boxAngle), s = sinf(m_boxAngle);
    for (auto& p : m_particles)
    {
    ...
    for (const auto& p : m_particles)
    {
        if (auto* e = m_entityManager->findEntity(p.entityName))
        {
            if (auto* s = e->getComponent<SpriteComponent>())
            {
                ...
                // Gradient: deep blue (slow) -> cyan -> white (fast)
...
void FlipFluidSimulationScene::renderMetaballs(GraphicsEngine& engine, DeviceContext& ctx)
{
    ...
    // Still render boundaries as normal sprites
    for (int i = 0; i < 4; ++i)
    {
    ...
    for (const auto& p : m_particles)
    {
        if (auto* e = m_entityManager->findEntity(p.entityName))
        {
            if (auto* s = e->getComponent<SpriteComponent>())
            {
                ...
                // This preserves the velocity coloring while creating metaball surfaces
                Vec4 particleColor = s->getTint();
                s->setTint(particleColor);
