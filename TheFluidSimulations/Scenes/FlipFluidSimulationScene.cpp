...
void FlipFluidSimulationScene::load(GraphicsEngine& engine)
{
    ...
    m_graphicsDevice = &static_cast<GraphicsDevice&>(device);
...
void FlipFluidSimulationScene::render(GraphicsEngine& engine, IRenderSwapChain& swapChain)
{
    auto& ctx = static_cast<DeviceContext&>(engine.getContext());
...
void FlipFluidSimulationScene::renderMetaballField(GraphicsEngine& engine, DeviceContext& ctx)
{
    ...
    if (!metaballTexture)
    {
        metaballTexture = Texture2D::LoadTexture2D(engine.getGraphicsDevice(), L"TheEngine/Assets/Textures/MetaballFalloff.png");
    }
